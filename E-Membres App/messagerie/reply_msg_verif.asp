...
if Request.Form("A") = session("user") Then
  Response.redirect "message_reply.asp?A=msg_reception&A2="&Request.Querystring("A2")&"&msg_ID="&Request.Querystring("msg_ID")&"&msg=!+Vous+ne+pouvez+pas+vous+envoyer+un+message+à+vous+même+!"
End if
...
if rSC.EOF Then
  Response.redirect "message_reply.asp?A=msg_reception&A2="&Request.Querystring("A2")&"&msg_ID="&Request.Querystring("msg_ID")&"&msg=!+Erreur+!"
End if
...
if session("A")="" OR session("objet")="" OR session("message")="" then
  Response.redirect "message_reply.asp?A=msg_reception&A2="&Request.Querystring("A2")&"&msg_ID="&Request.Querystring("msg_ID")&"&msg=!+Tous+les+champs+sont+obligatoire+!"
end if
...
if au = "" then
  Response.redirect "message_reply.asp?A=msg_reception&A2="&Request.Querystring("A2")&"&msg_ID="&Request.Querystring("msg_ID")&"&msg=!+L'utilisateur+ne+souhaite+pas+reçevoir+de+message+de+votre+part+!"
end if
